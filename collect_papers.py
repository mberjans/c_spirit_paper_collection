...
# Per-URL records: URL -> { doi, title, authors, year, venue, sources: set[str] }
URL_RECORDS_DICT: Dict[str, Dict[str, Any]] = {}
...
def update_url_record(url: str, source: Path) -> None:
    ...
    if rec is None:
        rec = {"doi": None, "title": None, "authors": None, "year": None, "venue": None, "sources": set()}
        URL_RECORDS_DICT[key] = rec
    # Fill DOI from URL if possible
    if not rec.get("doi"):
        doi = extract_doi_from_url(key)
        if doi:
            rec["doi"] = doi
            # If we have DOI metadata, copy into URL record if empty
            drec = DOI_IN_TEXT_DICT.get(doi)
            if drec:
                for fld in ("title", "authors", "year", "venue"):
                    if not rec.get(fld) and drec.get(fld):
...
def extract_metadata_from_folder(folder: Path, parse_control: Optional[ParseControl] = None) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], Optional[str], List[str]]:
    ...
    for dirpath, _, filenames in os.walk(folder):
        for fname in filenames:
            ...
            # Removed early exit to ensure scanning continues for links across all files
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--include-root",
        action="store_true",
        help="Also process each provided -d directory itself (not only its subfolders).",
    )
    parser.add_argument(
        "--follow-symlinks",
        action="store_true",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Preload previous document parse registry if provided
    if args.input_doc_registry:
        reg_path = Path(args.input_doc_registry).expanduser()
        if reg_path.exists():
            try:
                prev = json.loads(reg_path.read_text(encoding="utf-8"))
                if isinstance(prev, dict):
                    DOCUMENT_PARSE_REGISTRY.update(prev)
                else:
                    print(f"Warning: input doc registry at {reg_path} is not a JSON object; ignoring.", file=sys.stderr)
            except Exception as exc:
                print(f"Warning: failed to read input doc registry {reg_path}: {exc}", file=sys.stderr)
        else:
            print(f"Warning: input doc registry path does not exist: {reg_path}", file=sys.stderr)
    ...
    for root_dir in input_dirs:
        # Optionally process the root directory itself
        if args.include_root and _GLOBAL_PARSE_CONTROL.remaining_documents_to_parse > 0:
            try:
                summary, urls = summarize_folder(root_dir, root_dir)
            except Exception as exc:
                print(f"Error summarizing root {root_dir}: {exc}", file=sys.stderr)
            else:
                if not (args.only_with_pdfs and summary.num_pdfs < 1):
    ...
    # Write URL_RECORDS_DICT alongside URL dict if requested
    if args.output_url_dict:
        try:
            base = Path(args.output_url_dict).expanduser()
            out_path = base.parent / (base.stem + ".records.json")
            serializable = {}
            for url_key, rec in URL_RECORDS_DICT.items():
                serializable[url_key] = {
                    "doi": rec.get("doi"),
                    "title": rec.get("title"),
                    "authors": rec.get("authors"),
                    "year": rec.get("year"),
            ...
            if args.output_write_mode == "append" and out_path.exists():
                try:
                    existing = json.loads(out_path.read_text(encoding="utf-8"))
                    if isinstance(existing, dict):
                        # Merge prefer existing non-null; union sources
                        for url_key, old in existing.items():
                            curr = serializable.get(url_key)
                            if curr is None:
                                serializable[url_key] = old
                            else:
